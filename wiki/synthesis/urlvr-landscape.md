...
| [[wiki/papers/zhang-2026-praise\|PRAISE]] | Renmin U + Xiaohongshu | Prefix rollout reuse + adjacent prefix gains | 混合（GT answer scoring + prefix gains） | Multi-hop QA | 2026 |
| [[wiki/papers/chen-2026-slate\|SLATE]] | — | Prefix continuation + LLM judge dense reward | 混合（LLM judge） | 数学推理 | 2026 |
| [[wiki/papers/liang-2026-step\|STEP]] | UIUC | Hidden state step scorer + GPU-aware pruning | N/A（inference-time pruning） | 数学推理 | 2026 |
...
(dual-view)(dist)(topology)(PRM+F1)(tool+MV)(co-evo)
 Meta-TTRL V-Zero DistriTTRL  PRAISE  SLATE
(self-intro)(co-evo)(GMM conf)  (GT+prefix)(LLM judge)
  TTVS     DBB    CSRS
...
(grad decouple)(PIRL)(InfoNCE)(asym ent)(diversity)(online refine)(noise theory)
   SHAPE    STEP
(hier credit)(hidden state pruning)
...
| **步骤级 (Step-level)** | SPARK, PRISM, MCNIG, SHAPE, Self-Guide, PRAISE, SLATE | 评估每个推理步骤的正确性 |
...
| PRAISE | PPO (Proximal Policy Optimization) | Adjacent prefix gains: $r_t^{proc} = \alpha(v_t - v_{t-1})$ + rollout reuse |
| SLATE | GRPO + prefix continuation | Step-level advantage from shared prefix continuation + LLM judge |
| STEP | N/A（inference-time） | 2-layer MLP step scorer on hidden states + GPU memory-aware pruning |
...
### 13. Prefix-based rollout 是 step-level reward 的可行范式
**共识度**: ⭐⭐⭐⭐（PRAISE + SLATE + STEP 三篇从不同角度验证）

PRAISE 证明从 prefix 做 rollout 得到 intermediate answer 再计算 step reward 在 agentic search 中有效（5 个 benchmark 一致提升）。SLATE 从理论上证明 shared prefix continuation 的方差比 trajectory-level advantage 低 T 倍。STEP 证明 hidden states 在推理早期（前 25%步骤）就编码了 trace quality 信息，可以用轻量 MLP scorer 高效预测。三者共同支撑了 SPC 方案的核心假设：**从推理前缀处做探测（probing）来估计步骤质量是一个有效且有理论支撑的范式**。
...
Step-Level Credit Assignment
    ...
    │                    └── +3% acc, -30% tokens
    │
    ├── Prefix rollout 复用 ──→ PRAISE (Adjacent Prefix Gains)
    │                            └── 验证 prefix-based step reward 有效性
    │
    ├── Prefix continuation 方差降低 ──→ SLATE (Shared Prefix → k Continuations)
    │                                     └── Theorem: Var ↓ T倍
    │
    ├── Hidden state 早期预测 ──→ STEP (MLP Scorer on Hidden States)
    │                              └── 前25%步骤已能区分 correct/incorrect
    │
